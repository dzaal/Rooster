...
window.ROOSTER_CONFIG = {
  ...
  defaults: {
    ...
    // ICS-URL van de hoofdkalender met vrijwilligersdiensten
    calendarUrl: 'https://calendar.google.com/calendar/ical/DIENSTEN_ID%40group.calendar.google.com/public/basic.ics',
    ...
    // ── Laag 2: Afspraken ─────────────────────────────────────────────────
    // ICS-URL van een aparte agenda voor afspraken/bezoeken
    appointmentUrl: 'https://calendar.google.com/calendar/ical/AFSPRAKEN_ID%40group.calendar.google.com/public/basic.ics',

    // ── Laag 1: Hoofdevenementen (achtergrond) ────────────────────────────
    // ICS-URL van een agenda waarvan de events als achtergrondblok worden getoond
    ...
    // ── Woordenfilter ─────────────────────────────────────────────────────
    // Evenementen waarvan de naam één van deze woorden bevat worden nergens
    // weergegeven. Vergelijking is hoofdletterongevoelig.
...
**Vervanging:** als een naam in de agenda staat als `"Suzy ipv N"`, herkent de applicatie "Suzy" niet als crewlid. Los dit op door in Google Agenda de naam precies gelijk te stellen aan de crewnaam, of voeg een aparte entry toe:

```js
{ name: 'Suzy ipv N', color: '#52b788' }
```
...
Gebruik dit om:
- Terugkerende planningsblokken uit de hoofdevenementenkalender te verbergen
- Test-evenementen uit Google Agenda te filteren
- Interne notities die per ongeluk in een publieke kalender staan te onderdrukken
...
> **Let op:** de agenda moet openbaar zijn, of de proxy.php op de server moet toegang hebben. Privéagenda's werken niet via ICS zonder authenticatie.

---

## Schoolvakanties voor een andere regio instellen

Verwijder het commentaarteken voor `schoolHolidayCalendarUrl` en vul de ICS-URL in voor jouw regio:

```js
// Noord-Holland (standaard ingebouwd):
// schoolHolidayCalendarUrl: 'https://www.schoolvakanties-nederland.nl/ical/noord/',

// Utrecht, Gelderland, Overijssel, Flevoland (Midden):
schoolHolidayCalendarUrl: 'https://www.schoolvakanties-nederland.nl/ical/midden/',

// Noord-Brabant, Limburg, Zeeland (Zuid):
// schoolHolidayCalendarUrl: 'https://www.schoolvakanties-nederland.nl/ical/zuid/',
```

Als de externe URL niet beschikbaar is (CORS, netwerk), valt de app automatisch terug op de ingebouwde Amsterdamse data en verschijnt een waarschuwing in de browserconsole.
...
Zonder `appointmentUrl` of `mainEventCalendarUrl` worden die lagen simpelweg niet geladen.
